...
/// Gift$SVG widget.
/// {@endtemplate}
class Gift$SVG extends LeafRenderObjectWidget {
  /// {@macro Gift$SVG}
  const Gift$SVG({super.key, this.width, this.height, this.colorFilter});
  ...
  void updateRenderObject(
    ...
    Gift$SVGRenderObject renderObject,
  ) {
    renderObject
      ..width = width
      ..height = height
...
class Gift$SVGRenderObject extends RenderBox {
  Gift$SVGRenderObject();

  final _painter = _Gift$SVGPainter();
  ...
  Size computeDryLayout(BoxConstraints constraints) {
    final desiredWidth = _width ?? Gift$SVG.svgSize.width;
    final desiredHeight = _height ?? Gift$SVG.svgSize.height;
  ...
  void performLayout() {
    ...
    if (Gift$SVG.svgSize.width == 0 || Gift$SVG.svgSize.height == 0) {
      _scale = 1.0;
      return;
    }
    _scale = min(
      size.width / Gift$SVG.svgSize.width,
      size.height / Gift$SVG.svgSize.height,
    );
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final dx = (size.width - Gift$SVG.svgSize.width * scale) / 2;
    final dy = (size.height - Gift$SVG.svgSize.height * scale) / 2;
