...
/// Fire$SVG widget.
/// {@endtemplate}
class Fire$SVG extends LeafRenderObjectWidget {
  /// {@macro Fire$SVG}
  const Fire$SVG({super.key, this.width, this.height, this.colorFilter});
  ...
  void updateRenderObject(
    ...
    Fire$SVGRenderObject renderObject,
  ) {
    renderObject
      ..width = width
      ..height = height
...
class Fire$SVGRenderObject extends RenderBox {
  Fire$SVGRenderObject();

  final _painter = _Fire$SVGPainter();
  ...
  Size computeDryLayout(BoxConstraints constraints) {
    final desiredWidth = _width ?? Fire$SVG.svgSize.width;
    final desiredHeight = _height ?? Fire$SVG.svgSize.height;
  ...
  void performLayout() {
    ...
    if (Fire$SVG.svgSize.width == 0 || Fire$SVG.svgSize.height == 0) {
      _scale = 1.0;
      return;
    }
    _scale = min(
      size.width / Fire$SVG.svgSize.width,
      size.height / Fire$SVG.svgSize.height,
    );
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    final dx = (size.width - Fire$SVG.svgSize.width * scale) / 2;
    final dy = (size.height - Fire$SVG.svgSize.height * scale) / 2;
